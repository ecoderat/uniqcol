...
> **Not:** Dictionary encoding ve multi-segment merge bilinçli olarak bu prototipin dışında bırakılmıştır. Tek-kolonlu GROUP BY destekleniyor; çok-kolonlu GROUP BY, HAVING, ORDER BY ve AVG/MIN/MAX gibi ek aggregate'ler "Gelecek Çalışmalar"da listelenmiştir. Bkz. [Gelecek Çalışmalar](#gelecek-çalışmalar).
...
# Tek-kolonlu GROUP BY (COUNT(*) veya SUM(col))
./uniqcol query --db data/events.uniq \
  "SELECT country, COUNT(*) GROUP BY country"

./uniqcol query --db data/events.uniq \
  "SELECT country, SUM(amount) WHERE amount > 10.0 GROUP BY country"
...
```
Query("SELECT g, SUM(a) WHERE c = X GROUP BY g")
    │
    ▼
┌─────────────────────┐
│  Column Pruning     │ sadece a, c, g kolonları okunur
└─────────────────────┘
    │
    ▼
┌─────────────────────┐
│  RLE Decode + Scan  │ WHERE değerlendirilir (AND/OR ağacı)
│                     │ → seçim bit-vektörü
└─────────────────────┘
    │
    ▼
┌─────────────────────┐
│  Group / Aggregator │ GROUP BY varsa: per-grup birikim
│                     │ aksi halde: tek COUNT/SUM cevabı
└─────────────────────┘
```
...
### Okuma (Filtreli Scan ve Gruplu Sorgu)

| Senaryo | Süre | Throughput |
|---|---|---|
| `SELECT amount WHERE country = 'TR'` (1M satır, ~%5 seçicilik)        | 15.4 ms | 67.6 M rows/sec |
| `SELECT country, COUNT(*) GROUP BY country` (1M satır, 20 grup)        | 31.0 ms | 32.8 M rows/sec |
...
seçeceği için bunun kadar veya bundan hızlı olmalıdır). Gruplu sorgu
filtreli scan'in yaklaşık yarısı kadar throughput sağlıyor; sebep,
her satırın `map[any]*acc` aramasına girmesi ve grup anahtarının
`any`'ye kutulanması — bench çıktısındaki 1.95M alloc/op bunu
gösteriyor.
...
- **GROUP BY tek kolonludur.** `SELECT <col>, COUNT(*) | SUM(c) GROUP BY <col>` desteklenir; multi-kolonlu GROUP BY, HAVING, ORDER BY ve AVG/MIN/MAX yoktur.
...
- **Sorgu dili kısıtlıdır.** `query` alt komutu bilinçli olarak
  minimal tutulmuştur:
  - `SELECT <cols | * | COUNT(*) | SUM(col) | <col>, <AGG>>`
  - `[WHERE <cond> [(AND | OR) <cond>]* ]` — sınırsız sayıda koşul,
    `<cond>` = `<col> <op> <literal>`
  - `[GROUP BY <col>]` — yalnız tek kolon. Projeksiyon **tam olarak**
    `<group_col>, <AGG>` biçiminde olmalı; `AGG` = `COUNT(*)` veya
    `SUM(col)`. WHERE varsa, gruplandırmadan önce uygulanır.
  ...
  - Gruplu sonuçların satır sırası **keşif sırasıdır** (segment'te
    grubun ilk göründüğü sıra). Sıralı sonuç için bir gelecekteki
    `ORDER BY` gerekli.
  - **Desteklenmeyenler:** parantezler, `NOT`, iç içe ifadeler,
    kolon-kolona karşılaştırma (`WHERE a = b`), `IN` / `LIKE` /
    `BETWEEN`, `JOIN`, `HAVING`, `ORDER BY`, SQL içinde `LIMIT`,
    çok-kolonlu `GROUP BY`, birden fazla aggregate, `AVG` / `MIN` /
    `MAX`
...
4. **Sorgu motorunda eksik kalanlar** — multi-kolonlu `GROUP BY`, `HAVING`, `ORDER BY`, `AVG` / `MIN` / `MAX` ve aynı sorguda birden fazla aggregate. Tek-kolonlu `GROUP BY` ile `COUNT(*)` / `SUM(col)` ve tek-aggregate sorgular halihazırda çalışıyor.
